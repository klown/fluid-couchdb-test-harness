...
const { DataTypes } = require('sequelize-cockroachdb');  // SQL data types
...
fluid.each(
    ...
    },
    {
        modelName: "nonbulk",
        fields: {
            "name": { "type": DataTypes.STRING(36) },
            "awake": { "type": DataTypes.STRING(36) },
            "asleep": { "type": DataTypes.STRING(36) }
        }
    },
    {
        modelName: "supplemental",
        fields: {
            "_id": { "type": DataTypes.STRING(36), "primaryKey": true },
            "has": { "type": DataTypes.STRING(36) }
        }
    },
    {
        modelName: "users",
        fields: {
            "_id": { "type": DataTypes.STRING(64), "primaryKey": true },
    ...
    function (aModelDef) {
        module.exports[aModelDef.modelName+"TableModel"] = function (sequelize) {
            var retVal = {};
            retVal.modelName = aModelDef.modelName;
            retVal.model = sequelize.define(aModelDef.modelName, aModelDef.fields);
            return retVal;
        };
    }
);
