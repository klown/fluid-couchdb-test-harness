...
# Default values
COCKROACH_MAIN_CONTAINER=${COCKROACH_MAIN_CONTAINER:-"fluid-cockroachdb-test-harness"}
CONTAINER_LABEL=${CONTAINER_LABEL:-"$COCKROACH_MAIN_CONTAINER-foo"}
LISTEN_PORT=${LISTEN_PORT:-26257}
...
docker run -d \
    ...
    --hostname=roach1 \
    --net roachnet \
    -p $LISTEN_PORT:$LISTEN_PORT -p $COCKROACHDB_ADMIN_PORT:$COCKROACHDB_ADMIN_PORT \
